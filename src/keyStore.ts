...
interface KeyStoreInterface{
  ...
  getPrivateKeyData (keyId: string, password: string): any
  getRawKeyData (keyId: string): RawKeyData
  saveKey (keyId: string, password: string, privateData: any, publicData?: any): any
  saveKeys (data: {keyId: string, password: string, privateData: any, publicData?: any}[]): Promise<void>
...
class KeyStore implements KeyStoreInterface {
 ...
 static async createKeyStore (filePath, sodium: any)  {
  ...
  instanceSaveKey = (keyId: string, password: string, privateData: any, publicData?: any | {}): void => {
    ...
    async saveKey (keyID: string, password: string, privateData: any, publicData: any | {} = {}) {
      this.instanceSaveKey(keyID, password, privateData, publicData)
      await this.save(this.keysData)
    }
  
  
  saveKeys = async (data: { keyId: string; password: string; privateData: any; publicData?: any; }[]): Promise<void> => {
    try {
...
deriveHashFromPassword(password, metadata) {
...
encrypt(plaintext, metadata, password) {
    const secretKey = this.deriveHashFromPassword(password, metadata);
